...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description="Convert YOLO COCO 17 keypoints txt to Human3.6M 17 npz")
    parser.add_argument('--txt', type=str, default=None, help="Path to single yolo_skeleton.txt")
    parser.add_argument('--out', type=str, default=None, help="Output keypoints.npz path")
    parser.add_argument('--dir', type=str, default=None, help="Batch directory containing yolo_skeleton.txt files")
    args = parser.parse_args()

    if args.txt:
        out_path = args.out if args.out else os.path.join(os.path.dirname(args.txt), 'keypoints.npz')
        print(f"處理單一檔案: {args.txt} -> {out_path}")
        convert_txt_to_npz(args.txt, out_path)
    else:
        target_dir = args.dir if args.dir else r'D:\Pitt\Project\squat\demo'
        print(f"開始掃描資料夾: {target_dir}")
        txt_files = glob.glob(os.path.join(target_dir, '**', 'yolo_skeleton.txt'), recursive=True)
        if not txt_files:
            print(f"在 {target_dir} 中找不到任何 yolo_skeleton.txt 檔案。")
