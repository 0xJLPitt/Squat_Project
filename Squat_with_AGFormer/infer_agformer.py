...
if __name__ == '__main__':
    # 1. 權重路徑 (請確認您有下載 motionagformer-b-h36m.pth.tr 到 checkpoint 資料夾)
    checkpoint_file = r'D:\Pitt\Project\tools\MotionAGFormer\checkpoint\motionagformer-b-h36m.pth.tr'
    if not os.path.exists(checkpoint_file):
        print(f"錯誤：找不到模型權重 {checkpoint_file}")
        print("請參考 MotionAGFormer 的 README，下載 'MotionAGFormer-B' H3.6M weights 到 checkpoint 資料夾。")
        sys.exit(1)
        
    print("載入 MotionAGFormer 預訓練模型...")
    model = load_model(checkpoint_file)
    
    demo_dir = r'D:\Pitt\Project\squat\demo'
    npz_files = glob.glob(os.path.join(demo_dir, '**', 'keypoints.npz'), recursive=True)
    
    if not npz_files:
        print("找不到任何 keypoints.npz 檔案。請先執行 convert_keypoints.py")
        sys.exit(0)
    ...
    for npz in npz_files:
        ...
        # 這裡請確認您的原始影像長寬，否則正規化 (normalization) 會不準確
        w, h = get_video_resolution(demo_dir, npz)
