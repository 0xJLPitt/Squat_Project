...
呼叫指令: python step8_triangulate_and_render_3d.py
"""
import numpy as np
import cv2
import os
...
def remove_collapsed_joints(kpts_3d):
    pairs = [
        (15, 16), # Ankles
        (13, 14), # Knees
    ]
    ...
    for l_idx, r_idx in pairs:
        # Calculate initial distance from the first 50 frames (usually standing)
        valid_initial_dists = []
        for f in range(min(50, num_frames)):
            l_pt = fixed_kpts[f, l_idx]
            r_pt = fixed_kpts[f, r_idx]
            if np.sum(l_pt) != 0 and np.sum(r_pt) != 0:
                valid_initial_dists.append(np.linalg.norm(l_pt - r_pt))
                
        if len(valid_initial_dists) == 0:
            continue
            
        initial_dist = np.median(valid_initial_dists)
        
        # If they get closer than 40% of the initial distance, they are collapsed
        threshold = initial_dist * 0.4 
        if l_idx == 13: # Knees can naturally come closer (knee valgus), so threshold is lower
            threshold = initial_dist * 0.25
            
        for f in range(num_frames):
            l_pt = fixed_kpts[f, l_idx]
            r_pt = fixed_kpts[f, r_idx]
            if np.sum(l_pt) == 0 or np.sum(r_pt) == 0:
                continue
                
            dist = np.linalg.norm(l_pt - r_pt)
            if dist < threshold:
                # Collapsed due to occlusion! Set them to 0 so the interpolator can fill them in
                fixed_kpts[f, l_idx] = np.zeros(3)
                fixed_kpts[f, r_idx] = np.zeros(3)
