...
呼叫指令: python step11_project_raw_3d_to_video.py
"""
import numpy as np
import cv2
import os
from step8_triangulate_and_render_3d import (
    ...
    triangulate_point, cams, DATA_DIR
)

def get_raw_3d_keypoints(P, kpts_2d):
    max_frames = min([kpts_2d[c].shape[0] for c in cams])
    kpts_3d = np.zeros((max_frames, 17, 3))
    
    print("Triangulating (RAW, no filters)...")
...
def project_and_render(kpts_3d, P_matrices):
    ...
    for cam, name in cams.items():
        ...
        out_path = os.path.join(DATA_DIR, f"{name}_raw_projected.mp4")
        if not os.path.exists(vid_path):
            print(f"Video not found: {vid_path}")
            continue
            
        print(f"Projecting RAW 3D points onto {name}.avi...")
...
def main():
    ...
    kpts_3d = get_raw_3d_keypoints(P, kpts_2d)
    project_and_render(kpts_3d, P)
    
    print("All RAW projections completed!")
