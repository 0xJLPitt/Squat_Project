...
- **用途**：讀取 `S01_S108.json` 標註中的動作類別（`正常`、`下蹲深度不足`、`骨盆後傾`、`髖部上升過快(起身時髖部先啟動)`、`下蹲時髖部主導`、`下蹲時膝蓋主導`），分別獨立計算每個動作的切片誤差與總下數。
- **功能**：
...
```powershell
python autocutting.py
```
程式會自動拿腳本內寫死的 `DEFAULT_INPUT_PATH` 當作目標，掃描整個 `D:\squat_dataset` 資料夾。

#### 方式二：針對特定資料夾執行（參數覆蓋最高順位）
如果今天你剛拿到一位新受試者 (例如 S092) 的資料，或者只想測試單一影片，你可以透過 `--input` 參數從外部傳入路徑。**外部傳入的參數擁有最高順位，會直接覆蓋掉寫死的預設值**：
```powershell
# 針對特定受試者的母資料夾
python autocutting.py --input "D:\squat_dataset\S092\session01"

# 或者針對單一錄影資料夾
python autocutting.py --input "D:\squat_dataset\S092\session01\recording_20260515_110654"
```
