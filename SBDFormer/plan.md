...
* **$12$**：關節點數量（SBDFormer 臥推身體 12 關鍵點，已移除臉部眼睛耳朵 5 點；若使用全 COCO 則為 17 點）。
...
### 4. 關節點定義對齊（12 身體關鍵點 vs COCO 17）
* 本專案 YOLO-pose 經精簡後輸出 **身體 12 個關鍵點**（移除臉部眼睛與耳朵 5 點，保留肩、肘、腕、髖、膝、踝）。
* 製作 3D 骨架時，通常以 **「左髖（12 點格式中的點 6）與右髖（12 點格式中的點 7）的中點」** 作為虛擬的 Pelvis 骨盆中點（原 17 點格式則為點 11 與 12）。請確認模型設定與關節映射清單。
...
def create_agformer_dataset(video_2d_list, video_3d_list, window_size=81, stride=27, output_path="sbd_custom_dataset.npz"):
    ...
    for k2d, k3d in zip(video_2d_list, video_3d_list):
        ...
        # 2. 3D 骨盆中心化 (12 點格式中，以左髖 6 與右髖 7 的中點為原點；若為 17 點格式則為 11 與 12)
        hip_l_idx, hip_r_idx = (6, 7) if k3d.shape[1] == 12 else (11, 12)
        pelvis = (k3d[:, hip_l_idx:hip_l_idx+1, :] + k3d[:, hip_r_idx:hip_r_idx+1, :]) / 2.0
        k3d_relative = k3d - pelvis  # [Frames, num_joints, 3]
